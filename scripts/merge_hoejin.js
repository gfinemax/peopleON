/**
 * '김호진' 전용 정밀 병합 스크립트 + 전역 관계 테이블 업데이트
 */
...
async function mergeHoejin() {
    console.log('--- 김호진 병합 시작 ---');

    // 1. 김호진 데이터 3건 가져오기
    const { data: targets, error } = await supabase
        .from('account_entities')
        .select('*')
        .ilike('display_name', '김호진%');
    ...
    // Master 선정: 010-9182-1126 번호를 가지며 정보가 가장 많은 레코드
    const sorted = [...targets].sort((a, b) => {
        const aPhone = a.phone?.includes('010') ? 1 : 0;
        const bPhone = b.phone?.includes('010') ? 1 : 0;
    ...
    // Master 전화번호 포맷 통일 (010-9182-1126)
    await supabase.from('account_entities').update({ phone: '010-9182-1126' }).eq('id', master.id);

    // 2. 외래키가 걸려 있는 모든 테이블 목록 (에러 났던 payments 포함)
    ...
    for (const slave of slaves) {
        ...
        for (const req of tablesToUpdate) {
            // 해당 컬럼이 있는지 확인 차원에서 오류 무시 업데이트
            const { error: tErr } = await supabase.from(req.name).update({ [req.col]: master.id }).eq(req.col, slave.id);
            if (tErr && tErr.code !== 'PGRST205' && !tErr.message.includes('Columns not found')) {
                // 심각한 에러만 로그 남김
...
mergeHoejin().catch(console.error);
