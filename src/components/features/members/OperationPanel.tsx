...
export function LinkedOperationPanel({
    ...
    const hasRemainingSummary = totalRemainingRefund > 0 || totalExpectedRefund > 0 || totalPaidRefund > 0;
    const hasTopRemaining = topRemaining.length > 0;

    if (isCollapsed || (!hasRemainingSummary && !hasTopRemaining)) return null;
                        ...
                        {totalExpectedRefund > 0 && (
                            <div className="rounded-lg border border-[#3f5a32] bg-[#1a2a16] px-3 py-2">
                                ...
                                <p className="mt-1 text-sm font-bold text-emerald-100">
                                    {`${Math.round((totalPaidRefund / totalExpectedRefund) * 100)}%`}
                                </p>
                            </div>
                        )}
                    </div>
