...
import { findRosterColumn, parseRosterMoney, type RosterSyncRow } from '@/lib/members/memberRosterSync';

type RawRow = Record<string, unknown>;
type Preview = {
    ready: boolean;
    errors: string[];
    matched: Array<{ rowNumber: number; name: string; beforeUnitGroup: string | null; unitGroup: string; existingDetailTotal: number; targetTotalPaid: number; adjustmentAmount: number }>;
...
const PAYMENT_HEADERS = ['총납입금액', '총납부금액', '납입금액', '납부금액', '총납입액', '납입총액'];
...
export function MemberRosterSyncDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
    ...
    const [paymentColumn, setPaymentColumn] = React.useState('');
    const [preview, setPreview] = React.useState<Preview | null>(null);
    const [loading, setLoading] = React.useState(false);

    const reset = React.useCallback(() => {
        setFileName(''); setRawRows([]); setHeaders([]); setNameColumn(''); setUnitColumn(''); setPaymentColumn(''); setPreview(null);
    }, []);

    const rows = React.useMemo<RosterSyncRow[]>(() => {
        if (!nameColumn || !unitColumn || !paymentColumn) return [];
        return rawRows.map((row, index) => ({
            rowNumber: index + 2,
            name: String(row[nameColumn] ?? '').trim(),
            unitGroup: String(row[unitColumn] ?? '').trim(),
            totalPaid: parseRosterMoney(row[paymentColumn]),
        })).filter((row) => row.name);
    }, [nameColumn, paymentColumn, rawRows, unitColumn]);
    ...
    const onFile = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setPaymentColumn(findRosterColumn(discoveredHeaders, PAYMENT_HEADERS));
    };
    ...
    const requestPreview = async (apply = false) => {
        ...
        try {
            ...
            if (result.applied) {
                alert(`등기조합원 ${result.registeredCount}명, 환불조합원 ${result.refundCount}명과 총납입금액을 반영했어.`);
    ...
    const mappingReady = Boolean(nameColumn && unitColumn && paymentColumn && rows.length);
        ...
        <DialogContent className="flex max-h-[92vh] flex-col border-slate-800 bg-slate-900 text-slate-100 sm:max-w-[960px]">
            ...
            <div className="flex-1 space-y-4 overflow-hidden py-2">
                ...
                {headers.length > 0 ? <div className="grid grid-cols-3 gap-3">{[
                    ['이름 열', nameColumn, setNameColumn], ['평형 열', unitColumn, setUnitColumn], ['총납입금액 열', paymentColumn, setPaymentColumn],
                ...
                {preview ? <div className="space-y-3 overflow-hidden">
                    ...
                    <ScrollArea className="h-[300px] rounded border border-slate-700"><table className="w-full text-left text-xs"><thead className="sticky top-0 bg-slate-800 text-slate-400"><tr><th className="px-3 py-2">이름</th><th className="px-3 py-2">평형 변경</th><th className="px-3 py-2 text-right">기존 상세합계</th><th className="px-3 py-2 text-right">엑셀 총납입</th><th className="px-3 py-2 text-right">조정액</th></tr></thead><tbody>{preview.matched.map((row) => <tr key={row.rowNumber} className="border-t border-slate-800"><td className="px-3 py-2 font-bold text-white">{row.name}</td><td className="px-3 py-2">{row.beforeUnitGroup || '-'} → <b className="text-sky-300">{row.unitGroup}</b></td><td className="px-3 py-2 text-right tabular-nums">{money(row.existingDetailTotal)}</td><td className="px-3 py-2 text-right font-bold tabular-nums text-emerald-300">{money(row.targetTotalPaid)}</td><td className="px-3 py-2 text-right tabular-nums text-amber-300">{money(row.adjustmentAmount)}</td></tr>)}</tbody></table></ScrollArea>
                </div> : null}
            </div>
