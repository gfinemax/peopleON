    ...
    return (
        <header className="relative z-40 flex min-h-14 shrink-0 items-center gap-5 overflow-visible border-b border-white/[0.08] bg-[#081b2e] px-4 text-white sm:px-5">
            <div className="flex shrink-0 items-center gap-2 border-r border-white/10 pr-5">
                ...
                <span className="hidden text-[11px] font-bold text-slate-500 sm:inline">조합원 관리</span>
            </div>
                ...
                {MEMBER_WORKSPACE_PRESETS.map((preset) => {
                    ...
                    return (
                        <button
                            ...
                            aria-pressed={active}
                            className={cn(
                                'relative flex min-h-14 shrink-0 items-center gap-1.5 px-3 text-xs font-bold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400/70',
                                active ? 'text-cyan-300' : 'text-slate-400 hover:text-slate-100',
                            )}
                        >
                            <span>{preset.label}</span>
                            <span className={cn('rounded-full px-1.5 py-0.5 text-[10px]', active ? 'bg-cyan-400/10 text-cyan-300' : 'bg-white/5 text-slate-500')}>
            ...
            <div ref={pickerRef} className="relative shrink-0">
                <button
                    ...
                    className="inline-flex h-9 items-center gap-2 rounded-lg border border-cyan-400/20 bg-cyan-500/10 px-3 text-xs font-bold text-cyan-100 transition-colors hover:bg-cyan-500/15 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400/70"
                >
                    <MaterialIcon name="view_column" size="xs" />
                    <span className="hidden sm:inline">열 선택</span>
                    <span className="rounded-full bg-cyan-400/15 px-1.5 py-0.5 text-[10px]">{visibleColumns.length}</span>
                ...
                {pickerOpen ? (
                    <div role="menu" aria-label="표시할 열 선택" className="absolute right-0 top-11 z-50 w-64 rounded-xl border border-white/10 bg-[#10243a] p-2 shadow-2xl shadow-black/40">
                        <div className="px-2 py-2">
                            ...
                            <p className="mt-0.5 text-[10px] text-slate-500">한 개 이상의 열을 선택해 주세요.</p>
                        </div>
                        <div className="space-y-1">
                            {MEMBER_WORKSPACE_COLUMNS.map((column) => {
                                ...
                                return (
                                    <label key={column.id} className={cn('flex min-h-10 cursor-pointer items-center gap-2 rounded-lg px-2 text-xs transition-colors hover:bg-white/5', removalDisabled && 'cursor-not-allowed opacity-60')}>
