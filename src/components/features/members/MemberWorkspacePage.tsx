...
export function MemberWorkspaceSummary({ member, onEdit, onDelete, onPrint, onBack, onPhotoChanged }: { member: MemberDetailDialogMember; onEdit: () => void; onDelete: () => void; onPrint: () => void; onBack: () => void; onPhotoChanged?: () => void }) {
    ...
    return <div className="m-3 grid min-h-[82px] grid-cols-[minmax(300px,1.8fr)_repeat(6,minmax(92px,1fr))_auto] items-center rounded-lg border border-white/[0.06] bg-[#0d2942] px-4 shadow-lg shadow-black/10">
        <div className="flex min-w-0 items-center gap-3 border-r border-white/[0.07] pr-4"><MemberProfilePhoto memberId={member.id} name={member.name} hasImage={Boolean(member.profile_image_path)} onChanged={onPhotoChanged} /><div className="min-w-0"><div className="flex items-center gap-2"><h2 className="truncate text-2xl font-black text-white">{member.name}</h2><span className="whitespace-nowrap rounded bg-emerald-500/20 px-2 py-1 text-xs font-bold text-emerald-300">{member.status || '정상'}</span>{member.tiers?.slice(0, 2).map((tier) => <span key={tier} className="whitespace-nowrap rounded-full border border-cyan-400/15 bg-cyan-500/10 px-2 py-1 text-[10px] font-bold text-cyan-300">{tier}</span>)}</div><p className="mt-1 truncate whitespace-nowrap text-xs font-medium text-slate-400">회원번호 {member.member_number || '-'} <span className="mx-2 text-white/10">|</span> 연락처 {value(member.phone)}</p></div></div>
